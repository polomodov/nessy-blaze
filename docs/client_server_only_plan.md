...
- Tenant management HTTP payload contracts are tightened to active v1 keys:
  - org: `create(name|slug)`, `patch(name)`
  - workspace: `create(name|slug|type)`, `patch(name|slug)`
    Unsupported keys are rejected with `400 INVALID_PAYLOAD`.
- Browser backend tenant mapping no longer leaks path identifiers into org/workspace request payloads.
...
- Active chat UI style tokens are normalized to `assistant` naming (`assistant-bubble`), replacing residual legacy `agent` naming in core chat surfaces.
...
- Core web E2E suite now includes strict payload rejection checks for tenant org/workspace contract endpoints.
