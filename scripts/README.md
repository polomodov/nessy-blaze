...
1. `blaze-{version}-1.x86_64.rpm` (Linux RPM)
2. `blaze-{version}-full.nupkg` (Windows NuGet package)
3. `blaze-{version}.Setup.exe` (Windows installer)
4. `blaze-darwin-arm64-{version}.zip` (macOS Apple Silicon)
5. `blaze-darwin-x64-{version}.zip` (macOS Intel)
6. `blaze_{version}_amd64.deb` (Linux DEB)
