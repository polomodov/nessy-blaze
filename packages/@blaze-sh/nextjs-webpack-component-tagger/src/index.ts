...
/**
 * A webpack loader that adds `data-blaze-*` attributes to JSX elements.
 */
export default function blazeTaggerLoader(this: any, code: string) {
  ...
  const transform = async () => {
    try {
      ...
      walk(ast as any, {
        enter: (node: any) => {
          try {
            ...
            if (!loc) return;
            const blazeId = `${fileRelative}:${loc.line}:${loc.column}`;

            // Inject the attributes
            if (node.name.end != null) {
              ms.appendLeft(
                node.name.end,
                ` data-blaze-id="${blazeId}" data-blaze-name="${tagName}"`,
              );
              transformCount++;
            }
          } catch (error) {
            console.warn(
              `[blaze-tagger] Warning: Failed to process JSX node in ${this.resourcePath}:`,
              error,
            );
          }
        },
      ...
      console.warn(
        `[blaze-tagger] Warning: Failed to transform ${this.resourcePath}:`,
        error,
      );
      return null;
    }
  ...
  transform()
    ...
    .catch((err) => {
      console.error(`[blaze-tagger] ERROR in ${this.resourcePath}:`, err);
