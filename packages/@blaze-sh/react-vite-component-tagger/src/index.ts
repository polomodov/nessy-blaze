 ...
 */
export default function blazeTagger(): Plugin {
  return {
    name: "vite-plugin-blaze-tagger",
    apply: "serve",
    enforce: "pre",
    ...
    async transform(code: string, id: string) {
      try {
        ...
        walk(ast as any, {
          enter(node: any) {
            try {
              ...
              // ── 2. Check whether the tag already has data-blaze-id ───────────────
              ...
              console.warn(
                `[blaze-tagger] Warning: Failed to process JSX node in ${id}:`,
                error,
              );
            }
          },
        ...
        console.warn(
          `[blaze-tagger] Warning: Failed to transform ${id}:`,
          error,
        );
        return null;
      }
