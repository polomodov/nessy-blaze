...
import { VanillaMarkdownParser } from "./BlazeMarkdownParser";
import { CustomTagState } from "./stateTypes";
import { BlazeTokenSavings } from "./BlazeTokenSavings";

interface BlazeThinkProps {
  node?: any;
  children?: React.ReactNode;
}

export const BlazeThink: React.FC<BlazeThinkProps> = ({ children, node }) => {
  ...
  const tokenSavingsMatch =
    typeof children === "string"
      ? children.match(
          /^blaze-token-savings\?original-tokens=([0-9.]+)&smart-context-tokens=([0-9.]+)$/,
        )
      : null;
  ...
  // If it's token savings format, render BlazeTokenSavings component
