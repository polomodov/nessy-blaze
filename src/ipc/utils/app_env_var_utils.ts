...
function getEnvFilePath({ appPath }: { appPath: string }): string {
  return path.join(getBlazeAppPath(appPath), ENV_FILE_NAME);
}
...
export async function updateDbPushEnvVar({
  ...
  try {
    ...
    // Update or add BLAZE_DISABLE_DB_PUSH
    const existingVar = envVars.find(
      (envVar) => envVar.key === "BLAZE_DISABLE_DB_PUSH",
    );
    if (existingVar) {
      existingVar.value = disabled ? "true" : "false";
    } else {
      envVars.push({
        key: "BLAZE_DISABLE_DB_PUSH",
