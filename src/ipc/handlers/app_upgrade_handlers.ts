...
const availableUpgrades: Omit<AppUpgrade, "isNeeded">[] = [
  {
    ...
    description:
      "Installs the Blaze component tagger Vite plugin and its dependencies.",
    manualUpgradeUrl: "https://blaze.sh/docs/upgrades/select-component",
  },
  {
    id: "capacitor",
    ...
    manualUpgradeUrl:
      "https://blaze.sh/docs/guides/mobile-app#upgrade-your-app",
  },
];
...
function isComponentTaggerUpgradeNeeded(appPath: string): boolean {
  ...
  try {
    ...
    return !viteConfigContent.includes("@blaze-sh/react-vite-component-tagger");
  } catch (e) {
...
async function applyComponentTagger(appPath: string) {
  ...
  if (
    !content.includes(
      "import blazeComponentTagger from '@blaze-sh/react-vite-component-tagger';",
    )
  ) {
  ...
  if (content.includes("plugins: [")) {
    if (!content.includes("blazeComponentTagger()")) {
      content = content.replace(
        "plugins: [",
        "plugins: [blazeComponentTagger(), ",
      );
    }
  } else {
    throw new Error(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const process = spawn(
      "pnpm add -D @blaze-sh/react-vite-component-tagger || npm install --save-dev --legacy-peer-deps @blaze-sh/react-vite-component-tagger",
      {
        cwd: appPath,
        shell: true,
        stdio: "pipe",
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[blaze] add Blaze component tagger",
    });
...
async function applyCapacitor({
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[blaze] add Capacitor for mobile app support",
    });
