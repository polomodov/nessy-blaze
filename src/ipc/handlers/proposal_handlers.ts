...
async function getCodebaseTokenCount(
  ...
  const codebase = (
    await extractCodebase({
      appPath: getBlazeAppPath(appPath),
    ...
    try {
      ...
      if (
        ...
        const proposalSearchReplaceFiles =
          getBlazeSearchReplaceTags(messageContent);
        ...
        const proposalExecuteSqlQueries =
          getBlazeExecuteSqlTags(messageContent);
      ...
      if (latestAssistantMessage?.content) {
        const writeTags = getBlazeWriteTags(latestAssistantMessage.content);
        ...
        const commandTags = getBlazeCommandTags(latestAssistantMessage.content);
