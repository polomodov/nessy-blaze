(() => {
  ...
  function findElementByBlazeId(blazeId, runtimeId) {
    ...
    // Fall back to finding by blaze-id (will get first match)
    const escaped = CSS.escape(blazeId);
    return document.querySelector(`[data-blaze-id="${escaped}"]`);
  }
  ...
  function applyStyles(element, styles) {
    ...
    console.debug(
      `[Blaze Visual Editor] Applying styles:`,
      styles,
      "to element:",
      element,
    );
  ...
  function handleGetStyles(data) {
    ...
    const element = findElementByBlazeId(elementId, runtimeId);
    if (element) {
  ...
  function handleEnableTextEditing(data) {
    ...
    const element = findElementByBlazeId(componentId, runtimeId);
    if (element) {
  ...
  function handleGetTextContent(data) {
    ...
    window.parent.postMessage(
      {
        type: "blaze-text-content-response",
        componentId,
  ...
  window.addEventListener("message", (e) => {
    ...
    switch (type) {
      case "get-blaze-component-styles":
        handleGetStyles(data);
        break;
      case "modify-blaze-component-styles":
        handleModifyStyles(data);
        break;
      case "enable-blaze-text-editing":
        handleEnableTextEditing(data);
        break;
      case "disable-blaze-text-editing":
        handleDisableTextEditing(data);
        break;
      case "get-blaze-text-content":
